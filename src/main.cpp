...
int main() {
    ...
    RingBuffer<ImuSample> imu_latest;

    CameraProducer cam(cam_latest, 30);
    ImuProducer imu(imu_latest, 200);
    ...
    csv << "t_ns,cam_seq,cam_age_ms,imu_seq,imu_age_ms,llm_seq,llm_latency_ms\n";
    ...
    for (int i = 0; i < 30; i++) {
        ...
        auto imu_opt = imu_latest.peek(); // latest imu

        uint64_t cam_seq = cam_opt ? cam_opt->seq : 0;
        uint64_t imu_seq = imu_opt ? imu_opt->seq : 0;

        double cam_age_ms = cam_opt ? (double)(t0 - cam_opt->t_capture_ns) / 1e6 : -1;
        double imu_age_ms = imu_opt ? (double)(t0 - imu_opt->t_capture_ns) / 1e6 : -1;
        ...
        csv << t0 << "," << cam_seq << "," << cam_age_ms << "," << imu_seq << "," << imu_age_ms
            << "," << resp.seq << "," << llm_latency_ms << "\n";

        std::cout << "tick " << i << " cam_age_ms=" << cam_age_ms << " imu_age_ms=" << imu_age_ms
